...
cd "$(dirname "$0")/.."
(.local-build/Build/Products/Debug/Speek.app/Contents/MacOS/Speek -ApplePersistenceIgnoreState YES -speekPage "$PAGE" -speekWindowFrame "$FRAME" > /tmp/speek-run.log 2>&1 &)
sleep 4
